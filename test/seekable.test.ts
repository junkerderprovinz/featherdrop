// These tests pin down the seekable format's wire construction, since a change
// there breaks every existing share: nonce derivation, the AAD binding of index
// and finality, the layout and each tamper guarantee. They also check that a
// seek fetches only the covering chunks and that cf=1 blobs still decrypt.
...
/** The content region, all chunks concatenated, for a payload. */
...
/** A fetchCipherRange over an in-memory content region; the range is inclusive. */
...
test("deriveNonce: 24 bytes, first 16 unchanged, last 8 XOR big-endian counter", () => {
  ...
  // Big-endian, so the least significant byte is the last one.
...
test("deriveNonce: big-endian placement of a multi-byte counter", () => {
  // With a zero base the nonce's last 8 bytes are the counter itself.
  const base = new Uint8Array(24);
  // Within the safe-integer range but with bytes set in both 32-bit words.
  const safe = 0x0000_0001_0000_0001;
...
test("chunkByteRange: chunk i is at i*(PT_CHUNK+TAG) within the content region", () => {
  ...
  for (const i of [0, 1, 2, 10, 1000]) {
    ...
    // The caller clamps the end for a short final chunk.
...
test("exact multiple of PT_CHUNK emits no extra empty final chunk", async () => {
...
test("swap two chunks → throws (AAD index binding)", async () => {
  const data = bytes(PT_CHUNK * 2 + 10); // two full chunks and a short one
  const key = generateKey();
  const baseNonce = generateAeadBaseNonce();
  const cipher = await encryptContent(data, key, baseNonce);
  const stride = PT_CHUNK + CHUNK_TAG;
...
test("flip a non-final chunk to look final → throws (AAD finality)", async () => {
  const data = bytes(PT_CHUNK + 100);
  const key = generateKey();
  const baseNonce = generateAeadBaseNonce();
  const cipher = await encryptContent(data, key, baseNonce);
  const stride = PT_CHUNK + CHUNK_TAG;
  // Claiming a size of PT_CHUNK makes the decrypter expect chunk 0 to be
  // final, but it was sealed as non-final.
...
test("treat a final chunk as non-final → throws", async () => {
  // One final chunk, decrypted with a size that expects two.
...
test("range decrypt also rejects tamper within the fetched chunk", async () => {
  ...
  const bad = cipher.slice();
  const stride = PT_CHUNK + CHUNK_TAG;
  bad[2 * stride + 5] ^= 0xff; // inside chunk 2
...
test("every chunk of a file uses a unique (key, nonce) pair", async () => {
  // With a fresh key and base nonce per file, distinct chunk nonces rule out
  // (K, nonce) reuse, the one catastrophic AEAD failure.
...
test("a cf=1 secretstream blob decrypts through decryptWithKey", async () => {
