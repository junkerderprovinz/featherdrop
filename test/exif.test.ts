...
// A JPEG segment: marker, a length that includes its own two bytes, payload.
...
const APP1 = seg(0xe1, [0x45, 0x78, 0x69, 0x66, 0x00, 0x00, 9, 9]); // "Exif", where GPS lives
const APP13 = seg(0xed, [0x50, 0x68, 0x6f, 0x74, 0x6f]); // IPTC, "Photo"shop
const DQT = seg(0xdb, [0x00, 1, 2, 3]);
// Entropy-coded data follows SOS up to EOI and is copied as is.
const SOS_AND_DATA = [
...
test("a JPEG without metadata segments is returned unchanged", () => {
  ...
  assert.equal(bytes, input); // the same reference, nothing copied
});
...
test("image data after SOS is copied verbatim (incl. 0xFFE1-looking bytes)", () => {
  // Entropy-coded data can contain byte pairs that look like markers.
