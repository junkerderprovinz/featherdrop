...
- **Save the QR code as a PNG.** The share QR can now be downloaded as a crisp
  4× PNG (`components/ResultPanel.tsx`) — handy for printing or pasting into a
  chat. Pure client-side, no extra dependency or server round-trip.
- **Link previews (Open Graph / Twitter cards).** A shared link now renders a
  branded preview card with a 1200×630 image (`app/opengraph-image.png` /
  `app/twitter-image.png`, generated by `scripts/render-assets.mjs`) and matching
  `openGraph`/`twitter` metadata (`app/layout.tsx`). The card is deliberately
  generic — a `/d/<slug>` link never leaks the file's name (the download page
  sets no own metadata, so it inherits the generic card).
- **"Save QR code" translated across all 26 languages** (`result.downloadQr`).
